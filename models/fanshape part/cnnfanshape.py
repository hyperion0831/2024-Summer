...
class FanShapeConv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        # kernel_size是一个元组，例如 (5, 5)
        kernel_height, kernel_width = self.kernel_size
        # 创建卷积核权重
        self.kernel = self.add_weight(shape=(kernel_height, kernel_width, input_shape[-1], self.filters),
    ...
    def call(self, inputs):
        ...
        patch_size = kernel_height * kernel_width * channels

        # 将卷积核应用于补丁
        patches = tf.reshape(patches, (-1, num_patches, patch_size))
        conv_out = tf.matmul(patches, tf.reshape(self.kernel, (-1, self.filters)))
        conv_out = tf.reshape(conv_out, (batch_size, height, width, self.filters))
