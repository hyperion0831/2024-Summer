...
from scipy.ndimage import map_coordinates  # 添加此行
...
def cartesian_to_polar(image):
    ...
    # 使用 map_coordinates 进行双三次插值
    polar_image = map_coordinates(image, [r.ravel(), theta.ravel()], order=3).reshape(height, width)
...
model = Sequential([
    ...
    Conv2D(32, (3, 3), activation='relu', padding='same'),
    FanShapeConv2D(filters=64, kernel_size=(5, 5)),  # 修改卷积核大小
    Conv2D(32, (3, 3), activation='relu', padding='same'),
