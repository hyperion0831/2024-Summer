...
# 指定保存路径
save_path1 = 'C:/Users/25610/Desktop/online3/saveCylindrical/5,5new'
save_path2 = 'C:/Users/25610/Desktop/online3/saveCylindricalBilinear/5,5new'
save_path3 = 'C:/Users/25610/Desktop/online3/saveCylindricalBicubic/5,5new'# 新增路径
...
process_path(save_path3, std_devs3)  # 新增路径处理
...
filtered_fields3 = np.array([field for field in range(0, 151) if field in std_devs3])  # 新增过滤
...
plt.errorbar(filtered_fields1, filtered_std_devs1, yerr=filtered_errors1, fmt='o', linestyle='-', color='b', label='Circular')
plt.errorbar(filtered_fields2, filtered_std_devs2, yerr=filtered_errors2, fmt='o', linestyle='-', color='r', label='Bilinear')
plt.errorbar(filtered_fields3, filtered_std_devs3, yerr=filtered_errors3, fmt='o', linestyle='-', color='g', label='Bicubic')  # 新增绘图
plt.xlabel('Magnetic Field Parameter')
plt.ylabel('Average Standard Deviation (std)')
plt.title('Cylintrical Yes 5,5 Circular VS Bilinear VS Bicubic')
plt.grid(True)
plt.legend()

# 设置 y 轴范围从 0 开始
max_std_dev = max(max(filtered_std_devs1 + filtered_errors1), max(filtered_std_devs2 + filtered_errors2), max(filtered_std_devs3 + filtered_errors3))
